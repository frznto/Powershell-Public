function Stop-VROpsAdapterMonitoring {
    <#
    .SYNOPSIS
        Stops monitoring for a vROps adapter instance.
    .DESCRIPTION
        Calls PUT /suite-api/api/adapters/{adapterId}/monitoringstate/stop.
    .PARAMETER AdapterId
    ...
    .EXAMPLE
        Stop-VROpsAdapterMonitoring -AdapterId 'f3a1b2c3-...'
    .LINK
    ...
    process {
        if ($PSCmdlet.ShouldProcess($AdapterId, 'Stop adapter monitoring')) {
            $result = Invoke-VROpsApiRequest -Server $Server -SkipCertificateCheck:$SkipCertificateCheck `
                -Path   "adapters/$AdapterId/monitoringstate/stop" `
                -Method 'PUT'
